/**
 * ============================================================
 *  AnimalEye — AI Classifier  |  script.js
 * ============================================================
 *  Models (all PRETRAINED — zero training required):
 *    • MobileNet V2  (alpha=0.5)  — Top-5 label classification
 *    • COCO-SSD                   — Person + multi-object detection
 *
 *  Features:
 *    ✅ Image upload + drag-and-drop classification
 *    ✅ Human / person detection (COCO-SSD, indigo bounding box)
 *    ✅ Multi-animal detection with bounding boxes
 *    ✅ Top-5 prediction bars (animated)
 *    ✅ Confidence Chart.js bar chart
 *    ✅ Animal / Human Wikipedia fact card
 *    ✅ Live webcam classification (900 ms interval)
 *    ✅ Camera snapshot → classify in Upload panel
...
const BOX_COLORS = [
  '#A855F7', '#F97316', '#06B6D4', '#10B981', '#F43F5E',
  '#FBB045', '#EC4899', '#6366F1', '#14B8A6', '#84CC16'
];
...
const CHART_BG  = ['#C4B5FD', '#FBB045', '#5EEAD4', '#6EE7B7', '#FDA4AF'];
const CHART_BDR = ['#A855F7', '#F97316', '#06B6D4', '#10B981', '#F43F5E'];
...
async function loadModels() {
  try {
    ...
    banner.style.background   = 'linear-gradient(135deg,rgba(204,251,241,.7),rgba(186,230,253,.6))';
    banner.style.borderColor  = 'rgba(94,234,212,.45)';
    document.getElementById('lspinner').style.display = 'none';
    document.getElementById('banner-h').textContent   = '✅ MobileNet V2 + COCO-SSD Loaded';
    document.getElementById('banner-p').textContent   =
      'MobileNet for Top-5 labels · COCO-SSD for Humans & Objects · No training required';

    tryEnableClassify();
    showToast('🎉 Models ready! Upload a photo or start your webcam.');
...
async function classifyImage() {
  ...
  try {
    ...
    const msg = hasPerson
      ? `👤 ${personDets.length} person(s) detected!`
      : `✅ Done! ${detections.length} object(s) detected.`;
    showToast(msg);
...
function clearUploadResults() {
  ...
  document.getElementById('pred-list').innerHTML =
    '<div class="empty-hint"><span>🔭</span>Click Classify to analyse…</div>';
...
function resetUpload() {
  ...
  clearUploadResults();
  document.getElementById('pred-list').innerHTML =
    '<div class="empty-hint"><span>🔭</span>Upload an image to see predictions</div>';
...
async function startWebcam() {
  if (!modelReady) { showToast('⏳ Models still loading, please wait…'); return; }
  ...
  try {
    ...
    showToast('📷 Camera live! Detects humans & animals in real-time.');
...
function stopWebcam() {
  ...
  document.getElementById('cam-pred-list').innerHTML =
    '<div class="empty-hint"><span>📷</span>Start the webcam to see live results</div>';
...
function classifySnapshot() {
  ...
  showToast('📸 Snapshot loaded — classifying!');
}
...
function getPredColors(preds) {
  return preds.map((p, i) => {
    const isHuman = p.className.toLowerCase().includes('person') ||
                    p.className.toLowerCase().includes('human');
    if (isHuman) return { bg: '#818CF8', bdr: '#6366F1' };
    return {
...
function drawChart(preds, canvasId, key) {
  const isDark = document.documentElement.getAttribute('data-theme') === 'dark';
  const tc = isDark ? '#7A6E90' : '#9087A5';  // tick colour
  const gc = isDark ? 'rgba(100,80,180,.12)' : 'rgba(196,181,253,.15)';  // grid colour
  const tb = isDark ? 'rgba(30,24,48,.96)'   : 'rgba(255,255,255,.96)';  // tooltip bg
  const tt = isDark ? '#EDE9F6'              : '#2D1B4E';                 // tooltip title
...
function spawnConfetti() {
  const cols = ['#C4B5FD','#F9A8D4','#5EEAD4','#FBB045','#FDA4AF','#BAE6FD','#818CF8'];
  for (let i = 0; i < 34; i++) {
